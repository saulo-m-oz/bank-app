...
export const listAllTransactionsService = async (
  id: string,
  query: any
) => {
  ...
  if (query.date || query.type) {
    ...
    if (query.type && !query.date){
        const transactions = await transactionsRepo.createQueryBuilder("transaction").where(`transaction.${query.type} = :id`, {id: account.id}).getMany();
        return transactions;
    }
    ...
    if (query.date && !query.type){
        ...
        const filteredTransactions = transactions.filter(transaction => {
            if (transaction.createdAt.toLocaleDateString().includes(query.date)) return transaction;
        })
